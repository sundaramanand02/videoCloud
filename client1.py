...
def block_matching_and_error(frame1, frame2, block_size, search_range):
  """Performs block matching between two frames and calculates the error frame.
  ...
  Returns:
    A list of motion vectors, where each vector is a tuple of (x_displacement, y_displacement), and the error frame.
  """
  ...
  for y in range(0, height - block_size, block_size):
    for x in range(0, width - block_size, block_size):
      ...
      motion_vectors.append([x, y, best_match[0], best_match[1]])
      reconstructed_frame[y:y+block_size, x:x+block_size] = frame1[y+best_match[1]:y+best_match[1]+block_size, x+best_match[0]:x+best_match[0]+block_size]

  # Calculate the error frame
  error_frame = frame2 - reconstructed_frame

  return motion_vectors, error_frame
...
while cap.isOpened():
    ...
    if len(prev_frame) == 0:
        ...
        encrypted_frame_first = encrypt_npy_file(frame)
        frame_size_first = len(encrypted_frame_first)
        print(ct, frame_size_first)
        width_first, height_first, depth_first = frame.shape
        byte_width_height_first = struct.pack('LIII', frame_size_first, width_first, height_first, depth_first)
        # print(len(byte_width_height))
        client_socket.sendall(byte_width_height_first + encrypted_frame_first)
    ...
    motion_vec, error_frame = block_matching_and_error(prev_frame, gray_frame, block_size, max_displacement)
    prev_frame = gray_frame
    ...
    encrypted_frame = encrypt_npy_file(error_frame)
    encrypted_motion_vector = encrypt_npy_file(motion_vec)


    frame_size = len(encrypted_frame)
    motion_vector_size = len(encrypted_motion_vector)
    # print(ct, frame_size) 
    print(ct, frame_size,motion_vector_size)
    width, height = error_frame.shape 
    # width_mv, height_mv = motion_vec.shape
    byte_width_height = struct.pack('III', frame_size, width, height)
    
    # byte_width_height_mv = struct.pack('III', motion_vector_size, width_mv, height_mv)
    # print(len(byte_width_height))
    client_socket.sendall(byte_width_height + encrypted_frame)

    motion_vector_array = np.array(motion_vec)

    width_mv, height_mv =motion_vector_array.shape 
    byte_width_height_mv = struct.pack('III', motion_vector_size, width_mv, height_mv)

  

    client_socket.sendall(byte_width_height_mv + encrypted_motion_vector)
